  ...
  if (req.isAuthenticated()) {
      pool.connect(function (conErr, client, done) {
          if (conErr) {
              ...
              client.query("UPDATE mytasks SET user_task_description= $1 WHERE task_id = $2 AND usersapp_id = $3", [task_description, task_id, usersapp_id], function (queryErr, resultObj) {
                  done();
                  if (queryErr) {
                      res.sendStatus(500);
                  } else {
